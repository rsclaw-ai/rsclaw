...
import {
  ...
  getMessageThinkContent,
  getMessageContentOnly,
  isMessageThinking,
  getMessageTextContent as _getMessageTextContent,
  isDalle3,
  isVisionModel,
  safeLocalStorage,
  getModelSizes,
...
// RealtimeChat removed in rsclaw rebrand
import clsx from "clsx";

const localStorage = safeLocalStorage();

/** Extract `<rstools>` tool call log appended by the agent runtime. */
function extractRsTools(content: string): {
  cleanContent: string;
  rsTools: [string, string, string][];
} {
  const match = content.match(/<rstools>([\s\S]*?)<\/rstools>/);
  if (!match) return { cleanContent: content, rsTools: [] };
  try {
    const data: [string, string, string][] = JSON.parse(match[1]);
    return {
      cleanContent: content.replace(/<rstools>[\s\S]*?<\/rstools>/, "").trimEnd(),
      rsTools: Array.isArray(data) ? data : [],
    };
  } catch {
    return { cleanContent: content, rsTools: [] };
  }
}
  ...
  return (
    <div className={styles["chat-tool-block"]}>
      <div className={styles["chat-tool-block-header"]} onClick={() => setOpen((v) => !v)}>
        ...
        <span className={styles["chat-tool-block-toggle"]}>{open ? "▲" : "▼"}</span>
      </div>
      {open && (
        <pre className={styles["chat-tool-block-output"]}>{output}</pre>
      )}
    </div>
  );
}
  ...
  return (
    <div className="modal-mask">
      ...
      >
        <List>
          <ListItem
            title={Locale.Chat.Config.Topic.Title}
            subTitle={Locale.Chat.Config.Topic.SubTitle}
          >
            <input
              type="text"
              value={session.topic}
  ...
  return (
    <div className="modal-mask">
      ...
      >
        ...
        {/* Context prompts editor removed */}
      </Modal>
    </div>
  );
}
...
function ThinkBlock({ message }: { message: any }) {
  const isStreaming = message.streaming;
  const thinkContent = getMessageThinkContent(message);
...
function _Chat() {
  ...
  const chatCommands = useChatCommand({
    ...
    newm: () => { chatStore.newSession(); navigate(Path.Chat); },
  ...
  const onInput = (text: string) => {
    ...
    if (n === 0) {
      ...
      // Show /help commands instead of prompt search
      const cmds: RenderPrompt[] = [
        { title: "/help", content: getLang() === "cn" ? "\u663E\u793A\u53EF\u7528\u547D\u4EE4\u5217\u8868" : "Show available commands" },
        { title: "/reset", content: getLang() === "cn" ? "\u91CD\u7F6E\u5F53\u524D\u4F1A\u8BDD" : "Reset current session" },
        { title: "/new", content: getLang() === "cn" ? "\u65B0\u5EFA\u4F1A\u8BDD" : "Start a new session" },
        { title: "/model", content: getLang() === "cn" ? "\u5207\u6362\u6A21\u578B" : "Switch model" },
        { title: "/agent", content: getLang() === "cn" ? "\u5207\u6362\u667A\u80FD\u4F53" : "Switch agent" },
        { title: "/system", content: getLang() === "cn" ? "\u8BBE\u7F6E\u7CFB\u7EDF\u63D0\u793A\u8BCD" : "Set system prompt" },
        { title: "/clear", content: getLang() === "cn" ? "\u6E05\u9664\u4F1A\u8BDD\u5386\u53F2" : "Clear session history" },
  ...
  const onPromptSelect = (prompt: RenderPrompt) => {
    setTimeout(() => {
      ...
      // If selecting a / command, handle locally
      if (prompt.title.startsWith("/")) {
        ...
        if (cmd === "/help") {
          ...
          // Other commands: insert as text for user to edit/send
  ...
  // Only show bot hello if it has content.
  if (
    BOT_HELLO.content &&
    context.length === 0 &&
                        ...
                        <div className={styles["chat-message-container"]}>
                          ...
                          <div className={styles["chat-message-item"]}>
                            <Markdown
                              ...
                              content={extractRsFiles(extractRsTools(!isUser ? getMessageContentOnly(message) : getMessageTextContent(message)).cleanContent).cleanContent}
                              loading={
                          ...
                          {!isUser && !message.streaming && (() => {
                            ...
                            const { rsTools } = extractRsTools(raw);
                            const { rsFiles, rsImages } = extractRsFiles(raw);
                            return (rsTools.length > 0 || rsFiles.length > 0 || rsImages.length > 0) ? (
                              <>
                                {rsTools.length > 0 && (
                                  <div className={styles["chat-tool-blocks"]}>
                                    {rsTools.map(([name, args, output], idx) => (
                                      <ToolOutputBlock key={idx} name={name} args={args} output={output} />
                                    ))}
                                  </div>
                                )}
                          ...
                          <div className={styles["chat-message-action-date"]}>
                            {isContext
                              ...
                              : new Date(message.date).toLocaleString(getLang() === "cn" ? "zh-CN" : undefined)}
                          </div>
                        </div>
                      </div>
