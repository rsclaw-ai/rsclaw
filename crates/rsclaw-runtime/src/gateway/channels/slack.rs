...
pub(crate) fn start_slack_if_configured(
    ...
    use rsclaw_channel::slack::SlackChannel;
                    ...
                    if is_fast_preparse(&text) {
                        ...
                        tokio::spawn(async move {
                            ...
                            .await
                            {
                                ...
                                Ok(Err(_)) => {
                                    warn!("slack: chat-mode agent reply error");
                                    let _ = tx
                                        .send(OutboundMessage {
                                            target_id: channel_id.clone(),
                                            is_group: is_channel,
                                ...
                                Err(_) => {
                                    warn!("slack: chat-mode agent reply timed out");
                                    let _ = tx
                                        .send(OutboundMessage {
                                            target_id: channel_id.clone(),
                                            is_group: is_channel,
            ...
            loop {
                tokio::select! {
                    ...
                    () = cancel_for_out.cancelled() => {
                        info!("slack: channel cancelled, stopping outbound sender");
                        break;
                    }
                    msg = out_rx.recv() => {
        ...
        tokio::spawn(async move {
            tokio::select! {
                ...
                () = cancel_token.cancelled() => {
                    info!("slack: channel cancelled, stopping run loop");
                }
            }
        });
