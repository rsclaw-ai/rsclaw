...
pub(crate) fn start_dingtalk_if_configured(
    ...
    for (acct_name, app_key, app_secret, robot_code) in dt_accounts {
                    ...
                    if is_fast_preparse(&text) {
                        ...
                        tokio::spawn(async move {
                                ...
                                .await
                            {
                                ...
                                Ok(Err(_)) => {
                                    warn!("dingtalk: chat-mode agent reply error");
                                    let _ = tx
                                        .send(OutboundMessage {
                                            target_id: if is_group {
                                                conversation_id.clone()
                                            } else {
                                                sender_id.clone()
                                            },
                                ...
                                Err(_) => {
                                    warn!("dingtalk: chat-mode agent reply timed out");
                                    let _ = tx
                                        .send(OutboundMessage {
                                            target_id: if is_group {
                                                conversation_id.clone()
        ...
        let dt = Arc::new(rsclaw_channel::dingtalk::DingTalkChannel::new(
            app_key,
            app_secret,
            robot_code,
