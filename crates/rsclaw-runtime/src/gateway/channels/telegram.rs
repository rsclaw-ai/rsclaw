...
use rsclaw_channel::{Channel, OutboundMessage, telegram::TelegramChannel};
...
pub(crate) fn start_telegram_if_configured(
    ...
    let Some(tg_cfg) = &config.channel.channels.telegram else {
        return;
    };
    if !tg_cfg.base.enabled.unwrap_or(true) {
        return;
    }

    // Collect (account_name, bot_token) pairs from accounts.<name>.botToken.
    ...
    let group_allow_from: Vec<String> = tg_cfg.base.group_allow_from.clone().unwrap_or_default();
                    ...
                    if is_fast_preparse(&text) {
                        ...
                        tokio::spawn(async move {
                                ...
                                .await
                            {
                                ...
                                Ok(Err(_)) => {
                                    warn!("telegram: chat-mode agent reply error");
                                    let _ = tx
                                        .send(OutboundMessage {
                                            target_id: chat_id_s.clone(),
                                            is_group,
                                ...
                                Err(_) => {
                                    warn!("telegram: chat-mode agent reply timed out");
                                    let _ = tx
                                        .send(OutboundMessage {
                                            target_id: chat_id_s.clone(),
                                            is_group,
            ...
            loop {
                tokio::select! {
                    ...
                    () = cancel_for_out.cancelled() => {
                        info!("telegram: channel cancelled, stopping outbound sender");
                        break;
                    }
                    msg = out_rx.recv() => {
        ...
        tokio::spawn(async move {
            tokio::select! {
                ...
                () = cancel_token.cancelled() => {
                    info!("telegram: channel cancelled, stopping run loop");
                }
            }
        });
