...
pub async fn cmd_channels(sub: ChannelsCommand) -> Result<()> {
    ...
    match sub {
        ChannelsCommand::List | ChannelsCommand::Status => {
            ...
            let is_on = |b: Option<&rsclaw_config::schema::ChannelBase>| {
            ...
            match channel.as_str() {
                "wechat" | "weixin" | "openclaw-weixin" => {
                    ...
                    let (_url, qrcode) = if quiet {
                        rsclaw_channel::wechat::WeChatPersonalChannel::start_qr_login_silent(
                            &client,
                        )
                        .await?
                    } else {
                        rsclaw_channel::wechat::WeChatPersonalChannel::start_qr_login(&client)
                            .await?
                    };
                    ...
                    let (token, bot_id) =
                        rsclaw_channel::wechat::WeChatPersonalChannel::wait_qr_login(
                            &client, &qrcode,
                        )
                        .await?;
                            ...
                            if let Some(accounts) = accounts {
                                ...
                                acct.insert(
                                    ...
                                    serde_json::json!(format!(
                                        "WeChat {}",
                                        bot_id.chars().take(8).collect::<String>()
                                    )),
                                );
                            }
                        }
                ...
                "feishu" | "lark" | "openclaw-lark" => {
                    ...
                    let (app_id, app_secret, actual_brand) = if quiet {
                        rsclaw_channel::auth::feishu_auth::onboard_silent(&client, brand).await?
                    } else {
                    ...
                    if let Some(channels) = channels {
                        ...
                        if let Some(feishu) = feishu {
                            ...
                            feishu
                                .entry("brand")
                                .or_insert(serde_json::json!(actual_brand));
                            // Write credentials into next available account slot
                            let accounts = feishu
                                .entry("accounts")
                                .or_insert(serde_json::json!({}))
        ...
        ChannelsCommand::Unpair { channel, peer } => {
                ...
                .await
            {
                ...
                Ok(_) | Err(_) => {
                    ...
                    if let Ok(store) = rsclaw_store::redb_store::RedbStore::open(
...
/// If `default` is empty, use it. Otherwise create a new ID like
/// `{channel}-{timestamp}`.
...
fn rsclaw_allow_from_path(channel: &str) -> std::path::PathBuf {
    rsclaw_config::loader::base_dir().join(format!("credentials/{channel}-default-allowFrom.json"))
}
