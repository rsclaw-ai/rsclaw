...
pub async fn cmd_migrate(args: MigrateArgs) -> Result<()> {
    ...
    let oc_dir = if let Some(ref p) = args.openclaw_dir {
        let path = rsclaw_config::loader::expand_tilde_path_pub(p);
        if !path.is_dir() {
            err_msg(&format!(
    ...
    let rsclaw_dir = rsclaw_config::loader::base_dir();
...
async fn import_data(openclaw_dir: &PathBuf, rsclaw_dir: &PathBuf) -> Result<()> {
    ...
    match crate::gateway::startup::ensure_bge_model_present(&model_dir, None).await {
        Ok(()) => {
            match rsclaw_agent::memory::MemoryStore::open(
                &memory_data_dir,
                Some(&model_dir),
    ...
    if config_path.is_file() && !rsclaw_config.exists() {
        ...
        if let Some(agents) = config.get("agents") {
            ...
            let default_idx: Option<usize> = agents_cfg
                ...
                .and_then(|arr| {
                    arr.iter()
                        .position(|a| a.get("default").and_then(|d| d.as_bool()).unwrap_or(false))
    ...
    if let Some(agents) = config.pointer("/agents/list").and_then(|v| v.as_array()) {
        for agent in agents {
            ...
            match openclaw::convert_heartbeat(&src_workspace, &dst_workspace) {
                ...
                Err(e) => {
                    warn_msg(&format!(
                        "failed to convert HEARTBEAT.md for {agent_id}: {e}"
                    ));
                    stats.errors += 1;
                }
                _ => {}
    ...
    // approved peers in
    // `<openclaw>/credentials/<channel>-<account>-allowFrom.json`; rsclaw uses
    // the same format under `<rsclaw>/credentials/`, so a straight file copy
    // lights up the existing trust list on first run.
        ...
        if let Err(e) = std::fs::create_dir_all(&rs_creds) {
            ...
            for entry in entries.flatten() {
                ...
                match std::fs::copy(&src, &dst) {
                    Ok(_) => item(
                        "*",
                        &format!("allowFrom migrated: {name} ({count} entries)"),
                    ),
...
fn import_memories_to_redb(
    ...
    use rsclaw_agent::memory::MemoryDoc;
