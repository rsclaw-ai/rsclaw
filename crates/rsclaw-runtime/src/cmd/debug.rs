...
async fn dump_prompt_spec(args: DumpPromptSpecArgs) -> Result<()> {
    ...
    let ws_ctx = WorkspaceContext::load(
        &ws_dir,
        rsclaw_agent::workspace::SessionType::Normal,
        false,
    ...
    for entry in &config.agents.list {
        ...
        tool_defs.push(rsclaw_provider::ToolDef {
    ...
    if let Some(toolset_str) = toolset.as_deref() {
        ...
        if let Some(allowed) =
            rsclaw_agent::tools_builder::toolset_allowed_names(toolset_str, custom_tools)
        {
            tool_defs.retain(|t| allowed.contains(t.name.as_str()));
        }
    }

    let to_json = |t: &rsclaw_provider::ToolDef| {
        json!({
            "name": t.name,
    ...
    let rsclaw_version = rsclaw_provider::rsclaw::RSCLAW_DEFAULT_PREFIX_ID
        .split('/')
        .nth(1)
