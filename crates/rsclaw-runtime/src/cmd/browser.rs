...
pub async fn cmd_browser(sub: BrowserCommand) -> Result<()> {
    ...
    let mut session = if let Some(ws_url) = rsclaw_browser::detect_existing_chrome(ports).await {
        eprintln!("Connected to existing Chrome");
        rsclaw_browser::BrowserSession::connect_existing_reuse(&ws_url).await?
    } else {
        let chrome_path = rsclaw_agent::platform::ensure_chrome().await?;
        eprintln!("Launching headless Chrome");
        rsclaw_browser::BrowserSession::start(&chrome_path, false, None).await?
    };
