...
async fn plugins_install(spec: &str) -> Result<()> {
    ...
    if spec.starts_with("http://") || spec.starts_with("https://") {
        ...
        if rsclaw_skill::allowlist::snapshot().counts().1 == 0 {
...
async fn install_wasm_file(src: &std::path::Path) -> Result<()> {
    ...
    // can find it.
    let tmp_manifest = rsclaw_plugin::PluginManifest {
        name: stem.clone(),
        id: None,
        version: None,
        description: None,
    ...
    let wasm_plugin =
        rsclaw_plugin::load_wasm_plugin(&tmp_manifest, &engine, browser, None, None).await;
