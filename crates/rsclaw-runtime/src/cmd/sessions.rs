...
fn gateway_url() -> String {
    ...
    let port = rsclaw_config::loader::load_json5(&config_path)
        .ok()
...
fn gateway_token() -> String {
    let base = rsclaw_config::loader::base_dir();
    let config_path = base.join("rsclaw.json5");
    rsclaw_config::loader::load_json5(&config_path)
        .ok()
        .and_then(|c| {
            c.gateway
                .as_ref()?
...
pub async fn cmd_sessions(sub: SessionsCommand) -> Result<()> {
    match sub {
        ...
        SessionsCommand::Cleanup(_args) => {
            ...
            warn_msg(
                "cleanup via CLI is not supported while gateway is running. Use /clear or /new from chat.",
            );
        }
    }
    Ok(())
