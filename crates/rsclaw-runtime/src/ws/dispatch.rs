...
pub async fn dispatch(ctx: MethodCtx) -> MethodResult {
    ...
    match ctx.req.method.as_str() {
        ...
        "chat.permission_response" => methods::chat::chat_permission_response(ctx).await,
            ...
            let skills: Vec<serde_json::Value> = registry
                ...
                .map(|s| {
                    serde_json::json!({
                        ...
                        "source": rsclaw_config::loader::path_to_forward_slash(&s.dir),
                        "filePath": rsclaw_config::loader::path_to_forward_slash(&s.dir.join("SKILL.md")),
                        "baseDir": rsclaw_config::loader::path_to_forward_slash(&s.dir),
                        "path": rsclaw_config::loader::path_to_forward_slash(&s.dir),
                        "skillKey": s.name,
                        "bundled": false,
                        "always": false,
                        "disabled": false,
                ...
                .collect();
            Ok(serde_json::json!({
                "workspaceDir": rsclaw_config::loader::path_to_forward_slash(&global_dir),
                "managedSkillsDir": rsclaw_config::loader::path_to_forward_slash(&global_dir),
                "skills": skills,
            }))
        }
