...
impl AgentRuntime {
    ...
    pub(crate) async fn tool_web_browser(&self, ctx: &RunContext, args: Value) -> Result<Value> {
        ...
        if action == "capture_video" {
            ...
            let session = browser.as_mut().expect("browser initialized — checked above");
        ...
        let mut browser = self.browser.lock().await;
        let result = browser.as_mut().expect("browser initialized — checked above").execute(action, &args).await;
        ...
        if let Err(ref e) = result {
            ...
            if is_transport {
                ...
                return browser.as_mut().expect("browser initialized — checked above").execute(action, &args).await;
            }
        }
        result
    }
