...
impl AgentRuntime {
    ...
    pub(crate) async fn tool_stock_quote(&self, args: Value) -> Result<Value> {
        let arc = match rsclaw_astock::global_client() {
            Some(c) => c,
...
pub async fn render_chart_for_code(
    ...
    let charts_dir = rsclaw_config::loader::base_dir().join("var").join("charts");
    let filename = format!(
        "chart_{}_{}.png",
        code.replace(['.', '/', ':'], "_"),
    ...
    let opts = rsclaw_astock::chart::ChartOpts {
        title: title.clone(),
        subtitle: Some(subtitle),
        ma_periods,
        ..Default::default()
    };
    let size = rsclaw_astock::chart::render_kline_png(&kr.klines, &opts, &path)
        .map_err(|e| {
...
fn quote_to_json(q: &rsclaw_astock::Quote, format: &str) -> Value {
...
fn snapshot_sort_key(r: &rsclaw_astock::SnapshotRow, key: &str) -> f64 {
    match key {
        "price" => r.price,
        "pct" => {
...
/// match.
pub fn watchlist_scope(agent_id: &str, channel: &str, peer_id: &str) -> String {
...
pub async fn watchlist_list(
    mem: &std::sync::Arc<tokio::sync::Mutex<crate::memory::MemoryStore>>,
    scope: &str,
) -> Result<Value> {
    let store = mem.lock().await;
...
pub(crate) fn render_quotes_markdown(quotes: &[rsclaw_astock::Quote]) -> String {
...
pub fn render_quotes_markdown_with_names(
    quotes: &[rsclaw_astock::Quote],
    names: Option<&std::collections::HashMap<String, String>>,
) -> String {
    if quotes.is_empty() {
...
pub fn render_snapshot_markdown(
    rows: &[rsclaw_astock::SnapshotRow],
    used_watchlist: bool,
    sort_by: &str,
) -> String {
...
fn render_kline_summary_markdown(r: &rsclaw_astock::KlineResp) -> String {
...
mod tests {
    ...
    fn snapshot_sort_key_paths() {
        let r = rsclaw_astock::SnapshotRow {
            code: "X".into(),
            name: None,
            market: String::new(),
            price: 105.0,
    ...
    fn render_snapshot_markdown_used_watchlist() {
        let rows = vec![rsclaw_astock::SnapshotRow {
            code: "600519".into(),
            name: Some("贵州茅台".into()),
            market: "SH".into(),
            price: 1272.86,
    ...
    fn render_kline_summary_carries_range_and_change() {
        use rsclaw_astock::{KlineBar, KlineResp};
        let kr = KlineResp {
            code: "600519".into(),
            period: "1d".into(),
            adjust: "none".into(),
