...
fn builtin_defaults() -> String {
    crate::config::loader::embedded_defaults_toml().to_owned()
}
...
pub async fn cmd_setup(args: SetupArgs) -> Result<()> {
    ...
    // Language is already written into the config template via
    // default_config(lang).
    step(
        "+",
        &crate::i18n::t_fmt(
            ...
            STEP_USER_AGENT => {
                ...
                let default_ua = if user_agent.is_empty() {
                    if provider.user_agent.is_empty() {
                        crate::provider::DEFAULT_USER_AGENT.to_string()
                    } else {
