...
pub fn green(s: &str) -> String {
    if use_color() { format!("\x1b[32m{s}\x1b[0m") } else { s.to_owned() }
}

pub fn yellow(s: &str) -> String {
    if use_color() { format!("\x1b[33m{s}\x1b[0m") } else { s.to_owned() }
}

pub fn red(s: &str) -> String {
    if use_color() { format!("\x1b[31m{s}\x1b[0m") } else { s.to_owned() }
}

pub fn bold(s: &str) -> String {
    if use_color() { format!("\x1b[1m{s}\x1b[0m") } else { s.to_owned() }
}

pub fn dim(s: &str) -> String {
    if use_color() { format!("\x1b[2m{s}\x1b[0m") } else { s.to_owned() }
}

pub fn cyan(s: &str) -> String {
    if use_color() { format!("\x1b[36m{s}\x1b[0m") } else { s.to_owned() }
}
