...
// Build result with optional hints/parsed errors on failure
            let mut result = json!({
                "task_id": task_id,
            ...
            let is_error = output.status.code().map(|c| c != 0).unwrap_or(true);
            if is_error && !stderr.is_empty() {
                ...
                result["error_parsed"] = json!(parsed_error);
                // Extract CLI hint for did-you-mean suggestions
                if let Some(hint) = extract_cli_hint(&stderr) {
