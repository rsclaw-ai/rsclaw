...
impl WeComChannel {
    ...
    /// Send a typed media message (voice/video) using a previously uploaded
    /// `media_id`.
    async fn send_typed_message(
        ...
        body.as_object_mut()
            .expect("json object")
            .insert(msg_type.to_owned(), media_obj);
        let frame = json!({
            "cmd": "aibot_send_msg",
            "headers": { "req_id": &req_id },
            "body": body,
                    ...
                    if path_or_url.starts_with("http://") || path_or_url.starts_with("https://") {
                            ...
                            .await
                        {
                            Ok(resp) if resp.status().is_success() => match resp.bytes().await {
                                ...
                                _ => {
                                    warn!(idx, "WeCom: empty file download");
                                    continue;
                                }
                            },
                            _ => {
                                warn!(idx, "WeCom: file download failed: {path_or_url}");
                                continue;
                            }
                        }
                    } else {
                        match std::fs::read(path_or_url) {
                            Ok(b) => b,
                            Err(e) => {
                                warn!(idx, "WeCom: failed to read file {path_or_url}: {e}");
                                continue;
                            }
                        }
                    };
                ...
                match self.upload_media(&bytes, media_type, filename).await {
                    Ok(media_id) => {
                        let send_result = match msg_type {
                            "video" => {
                                self.send_typed_message(&msg.target_id, "video", &media_id)
                                    .await
                            }
                            "voice" => {
                                self.send_typed_message(&msg.target_id, "voice", &media_id)
                                    .await
                            }
