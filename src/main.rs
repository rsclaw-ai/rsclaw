...
fn main() -> Result<()> {
    ...
    // Use a thread with larger stack (8MB) to avoid stack overflow in debug builds.
    // The main thread's default stack on Windows is ~1MB which can overflow
    // with large async state machines generated by complex CLI dispatch.
    sys::run_with_large_stack(move || {
