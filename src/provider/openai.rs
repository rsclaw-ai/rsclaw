...
impl LlmProvider for OpenAiProvider {
    ...
    fn stream(&self, req: LlmRequest) -> BoxFuture<'_, Result<LlmStream>> {
        Box::pin(async move {
            // Log max_tokens being sent to LLM
            tracing::info!(
                model = %req.model,
...
mod tests {
    ...
    async fn sse_line_buffer_handles_split_lines() {
        ...
        // First chunk: incomplete line (no newline)
        // The complete JSON will be: {"choices":[{"delta":{"content":"hel"}}]}
        // We split in the middle of the content value
        let chunk1 = Ok(bytes::Bytes::from(
            r#"data: {"choices":[{"delta":{"content":"he"#,
        ));
        let events1 = parse_sse_chunk_with_buffer(chunk1, &buffer).await;
        // Should return no events (incomplete line)
        ...
        // Should return one TextDelta event with "hel"
    ...
    async fn sse_line_buffer_handles_trailing_incomplete_line() {
        // Test that incomplete trailing line is buffered
        let buffer = tokio::sync::Mutex::new(String::new());
        ...
        let events1 = parse_sse_chunk_with_buffer(chunk1, &buffer).await;
        // Should return only the first complete event
        assert_eq!(events1.len(), 1);

        // Second chunk completes the trailing line
